...
resourcename="stocks"  # lower case, only
resourcegroup="stocks_rg"
...
# git clone https://github.com/apilogicserver/stocks.git
# cd stocks
...
az group create --name stocks_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group stocks_rg --sku S1 --is-linux
az container create --resource-group stocks_rg --name stocks --image apilogicserver/stocks:latest --dns-name-label stocks --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//stocks.westus.azurecontainer.io:5656'
